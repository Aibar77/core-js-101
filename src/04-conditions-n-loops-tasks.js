...
function isCreditCardNumber(ccn) {
  const cardDigits = ccn.toString().split("").map(Number);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (keys.includes(str[i])) {
      if (str[i] == queue.at(-1) && obj[str[i]] == str[i]) {
        queue.pop();
        continue;
      }
      queue.push(str[i]);
    } else if (str[i] == obj[queue.at(-1)] && queue.length > 0) {
      queue.pop();
    } else {
      return false;
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPaths[0].length; i++) {
    ...
    if (splitPaths.every((pathParts) => pathParts[i] === currentPart)) {
